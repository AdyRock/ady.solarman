...
class Inverter
{
    ...
    {
        var serial_hex = value.toString(16).padStart(size, 0);
        var serial_bytes = [];
        for (var c = 0; c < serial_hex.length; c += 2)
        {
    ...
    get_read_business_field(start, length, mb_fc)
    {
        ...
        let crc = this.getModbusChksum(request_data);
    ...
    generate_request(start, length, mb_fc)
    {
        ...
        let business_field = this.get_read_business_field(start, length, mb_fc);
    ...
    validateMODBUSData(MODBUSPacket, mb_functioncode)
    {
        ...
        if ((MODBUSPacket[0] === 1) && (MODBUSPacket[1] === mb_functioncode))
        {
            ...
            let byteCountRequired = MODBUSPacket[2];
            let byteCountReceived = MODBUSPacket.length - 5;
            ...
            let modbusData = MODBUSPacket.subarray(0, 3 + byteCountRequired);   // Get the MODBUSS packet (without the checksum)
            let chkSumCalc = this.getModbusChksum(modbusData);                  // Calculate the checksum
            let chkSumRx = MODBUSPacket.readUInt16LE(3 + byteCountRequired);    // Extract the packet checksum
    ...
    async send_request(request, start, end, mb_fc)
    {
        var result = 0;
        var length = end - start + 1;
        var requestData = this.generate_request(start, length, mb_fc);
        ...
        return new Promise((resolve, reject) =>
        {
            ...
            conn.on('data', (data) =>
            {
                ...
                let modbusSection = data.subarray(25, data.length - 2);
                returnData.push(modbusSection);
                let modbusPacket = Buffer.concat(returnData);
                
                console.log("Processing ", modbusPacket);

                let bytesToProcess = this.validateMODBUSData(modbusPacket, request.mb_functioncode);
                if (bytesToProcess < 0)
                {
                    // Invalid data
                ...
                if (bytesToProcess > 0)
                {
                    ...
                    let modbusData = modbusPacket.subarray(3, 3 + bytesToProcess);
                    conn.destroy();
                    resolve(modbusData);
                    return;
                }
...
//                resolve(Buffer.concat(returnData));
    ...
    {
        var result = true;
        var params = new ParameterParser(this.parameter_definition);
        for (var request of this.parameter_definition.requests)
        {
            try
            {
                this._current_val = await this.send_request(request, request.start, request.end, request.mb_functioncode);
