...
class OAuth2Device extends Homey.Device {
  ...
  async onInit() {
    ...
    if (typeof this.onOAuth2Migrate === 'function') {
      try {
        ...
        this.setUnavailable('Migration failed. Please re-authorize.').catch(this.error);
        this.error(err);
        return;
      }
    }
  ...
  async onOAuth2Destroyed() {
    this.setUnavailable('The session has been revoked. Please re-authorize.').catch(this.error);
  }
  ...
  async onOAuth2Expired() {
    this.setUnavailable('The session has expired. Please re-authorize.').catch(this.error);
  }
